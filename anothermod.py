...
from tkinter import messagebox, Tk, Toplevel, StringVar, Label, Entry, Button, Radiobutton, IntVar, Canvas, Frame, OptionMenu
import hashlib
import sqlite3
import os
...
class QuizApp:
   ...
   def merge_sort(self, arr, ascending=True):
    ...
    if len(arr) > 1:
        ...
        while i < len(left_half) and j < len(right_half):
            # If the current element of the first half is smaller (or larger, for descending order)
            if (ascending and left_half[i] < right_half[j]) or (not ascending and left_half[i] > right_half[j]):
                # Put it in the merged array
                arr[k] = left_half[i]
                i += 1
            else:
                # Otherwise, put the current element of the second half in the merged array
                arr[k] = right_half[j]
                j += 1
            k += 1

        # If there are remaining elements in the first half, add them to the merged array
        while i < len(left_half):
            arr[k] = left_half[i]
            i += 1
            k += 1

        # If there are remaining elements in the second half, add them to the merged array
   ...
   def summary(self):
    ...
    # Create a dropdown menu for sort order
    sort_order = StringVar()
    sort_order.set("Ascending")  # default value
    dropdown = OptionMenu(summary_window, sort_order, "Ascending", "Descending")
    dropdown.pack()

    # Sort the scores according to the selected order
    self.merge_sort(previous_scores, sort_order.get() == "Ascending")
